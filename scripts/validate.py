...
def readiness(project_path: Path = PROJECT_PATH, mode: str = "development", project_root: Path | None = None) -> dict:
    ...
    project_root = (project_root.resolve() if project_root is not None else project_path.parent)
...
def bundle_checks(root: Path = ROOT) -> list[Check]:
    ...
    if contract is not None and schema is not None:
        ...
        integrity_ok = (
            ...
            and integrity.get("release_provenance") == {"mechanism":"github_artifact_attestation","workflow":".github/workflows/release.yml","attestation_action":"actions/attest@1e69f48acb82d1966a394da916b4c1698aa569d6","signature_model":"sigstore_oidc","required_source_ref":"refs/heads/main","verifier_requires_expected_source_digest":True,"subject_artifacts":["universal-agent-docs.zip","universal-agent-docs.sha256","universal-agent-docs.trust.json","universal-agent-docs.release.json","universal-agent-docs-consumer.zip","universal-agent-docs-consumer.sha256","universal-agent-docs-consumer.release.json"]}
        )
        checks.append(Check(
        ...
        consumer_ok = (
            ...
            and consumer.get("project_file_default") == "PROJECT.md"
            and consumer.get("project_root_default") == "."
...
def main() -> int:
    ...
    parser.add_argument("--project-file", type=Path, help="project facts Markdown for --readiness; defaults to PROJECT.md in the policy source tree, or PROJECT.md under --project-root")
    parser.add_argument("--project-root", type=Path, help="repository root used to resolve readiness evidence, component paths, and Git revision")
    ...
    if (args.project_file or args.project_root) and not args.readiness:
    ...
    if args.readiness:
        ...
        r = readiness(project_file, args.readiness, project_root=project_root)
        result["readiness"] = r
