...
> **Source layout note:** canonical source template은 `templates/PROJECT.md`다. repository root의 `PROJECT.md`는 기존 source/validator 호환성을 위한 byte-for-byte mirror이며 두 파일은 CI에서 exact parity를 검증한다. consumer package는 canonical template을 `.agent-policy/PROJECT.md`로 materialize한다.

이 파일은 `PROJECT_MAP.md`와 `ARCHITECTURE.md` 역할을 통합한 단일 프로젝트 지도다. 상세 제품 명세나 장기 계획을 복제하지 말고 실제 Source of Truth의 위치를 가리킨다.

이 template은 consumer 프로젝트가 복사·검토해 채우기 위한 것이며 `universal-agent-docs` upstream 자체의 운영 readiness를 기록하지 않는다. 따라서 `profile: "template"`과 Unknown fact가 남아 있는 것이 의도된 초기 상태다. consumer 설치에서는 `.agent-policy/PROJECT.md`를 편집하고 `--project-root <consumer-repo>`로 실제 프로젝트의 경로/Git evidence를 검증한다.
