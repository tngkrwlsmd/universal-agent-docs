...
GitHub 저장소에서는 `.github/workflows/release.yml`을 수동 실행하면 source artifact 4개와 consumer artifact 3개를 모두 생성하고 workflow artifact로 먼저 업로드한 뒤, full-SHA로 pin한 `actions/attest`로 **7개 artifact 전체**에 GitHub Artifact Attestation을 만든다. 이는 OIDC 기반 Sigstore 서명으로 build provenance를 제공한다. attestation 단계가 실패하더라도 이미 생성·업로드된 unsigned artifact는 troubleshooting evidence로 남을 수 있지만, release workflow 자체는 실패하며 `verify-release.yml`이 신뢰 가능한 release로 승인하지 않는다.

`.github/workflows/verify-release.yml`은 release workflow run ID와 **별도 trusted channel에서 얻은 40-hex `expected_source_sha`**를 함께 입력받는다. verifier는 해당 run이 `.github/workflows/release.yml`의 성공한 `workflow_dispatch`이고 `main`에서 실행되었는지 확인한 뒤 run의 `head_sha`가 외부 expected SHA와 정확히 같은지 검증한다. 그 후 source 4개와 consumer 3개 **모든 release artifact**에 대해 `gh attestation verify`로 repository, signer workflow, `refs/heads/main`, source digest를 함께 고정한다. source ZIP은 detached checksum + core/full manifests로, consumer ZIP은 detached checksum + consumer release manifest와 consumer verifier로 각각 검증한다. run 자신이 제공한 SHA만으로 trust expectation을 만들지 않는다.
